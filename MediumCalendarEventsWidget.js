  ...
  async function setBackground(widget, { useBackgroundImage, backgroundColor }) {
    if (useBackgroundImage) {
      ...
      if (exists && config.runsInWidget) {
        ...
        bgColor.colors = [backgroundColor, backgroundColor];
    ...
    combinedEvents.forEach((event) => {
      ...
      if (start <= inNumHours) {
        if (event.isAllDay) {
          ...
          const hourKey = HOUR_FORMAT.format(start < now ? now : start);
          ...
          if (start < now) {
            eventDuration = eventDuration - (Math.floor(((now - start) / 1000) / 60));
          }
